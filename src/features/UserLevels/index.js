...
export default class UserLevelsFeature extends Feature {
    ...
    async handleLevelCommand(interaction) {
        ...
        const levelObj = await this.userLevelManager.getObj(interaction.user.id, interaction.guild.id)
    ...
    onLevelUp = async (user_id, guild_id, currentLevelObj) => {
        ...
        if (guildLevelsConfig.notifyChannelId) {
            const channel = await this.bot.client.channels.fetch(guildLevelsConfig.notifyChannelId)

            let embed = new EmbedBuilder()
                .setTitle("Level up!")
                .setDescription(`Congratulations <@${user_id}>, you are now level [${currentLevelObj.level}]!`)
                .setTimestamp()
    ...
    handleNextLevel = async (user_id, guild_id) => {
        const guildLevelsConfig = await this.guildsConfigCache.get(guild_id)
        const currentLevelObj = await this.userLevelManager.getObj(user_id, guild_id)

        let targetLevel = getTargetLevelFromPoints(currentLevelObj.points, guildLevelsConfig.levels)

        if (currentLevelObj.level !== targetLevel.level) {
            await this.userLevelManager.updateLevel(user_id, guild_id, targetLevel.level)
    ...
    async handleOnMessage(message) {
        ...
        const currentLevel = await this.userLevelManager.getObj(message.author.id, message.guild.id)
        ...
        this.console.info(`[${message.author.username}] Points update due a message [${currentLevel.points}]+${earning.points} = [${nextPoints}]`)

        await this.userLevelManager.updatePoints(message.author.id, message.guild.id, nextPoints)
        await this.userLevelManager.updateLastInteraction(message.author.id, message.guild.id, {
            date: new Date(),
            type: "message",
